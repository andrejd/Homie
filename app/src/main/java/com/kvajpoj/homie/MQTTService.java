...
public class MQTTService extends Service {
    ...
    private class MqttEventCallback implements MqttCallback {
        ...
        public void deliveryComplete(IMqttDeliveryToken arg0) {
            Log.v(TAG, "onDeliveryComplete()");
        }

        @Override
        @SuppressLint("NewApi")
        public void messageArrived(String topic, final MqttMessage msg) throws Exception {
            Log.i(TAG, "Message arrived from topic " + topic + " " + msg.toString());
    ...
    private void doConnect(){
        ...
        try {
            mqttClient = new MqttAsyncClient("tcp://192.168.1.210:1883", deviceId, new MemoryPersistence());
            token = mqttClient.connect();
            token.waitForCompletion(3500);
            mqttClient.setCallback(new MqttEventCallback());
            token = mqttClient.subscribe("hello", 0);
